...
function update_address(hash, txid, amount, type, other_info, cb) {
		...
		if (address) {
			...
			if ( hash == 'coinbase' ) {
				...
				lib.is_unique(address.txs, txid, function(unique, index) {
					...
					if (unique == true) {
						tx_array.push({addresses: txid, type: type, amount: amount, time: other_info.time, block: other_info.block});
			...
			if (type == 'vin') {
				var newAddress = new Address({
					a_id: hash,
					txs: [ {addresses: txid, type: 'vin', amount: amount, time: other_info.time, block: other_info.block} ],
					sent: amount,
					balance: amount,
				});
			} else {
				var newAddress = new Address({
					a_id: hash,
					txs: [ {addresses: txid, type: 'vout', amount: amount, time: other_info.time, block: other_info.block} ],
					received: amount,
					balance: amount,
				});
			}
					...
					lib.prepare_vin(tx, function(vin) {
						lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
							lib.syncLoop(vin.length, function (loop) {
								...
								update_address(nvin[i].addresses, txid, nvin[i].amount, 'vin', {time: tx.time, block: block.height}, function(){
									loop.next();
								});
							}, function(){
								lib.syncLoop(vout.length, function (subloop) {
									...
									if (vout[t].addresses) {
										update_address(vout[t].addresses, txid, vout[t].amount, 'vout', {time: tx.time, block: block.height}, function(){
											subloop.next();
										});
									} else {
										subloop.next();
